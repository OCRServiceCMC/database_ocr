...
CREATE TABLE [UserRoleMappings] (
  ...
  FOREIGN KEY ([RoleID]) REFERENCES [UserRoles]([RoleID]) -- Khóa ngoại liên kết với bảng UserRoles
);
...
CREATE TABLE [ContractStatusHistory] (
  ...
  FOREIGN KEY ([ContractID]) REFERENCES [ServiceContracts]([ContractID]) -- Khóa ngoại liên kết với bảng ServiceContracts
);
...
CREATE TABLE [ProcessingLogs] (
  ...
  FOREIGN KEY ([FileID]) REFERENCES [UploadedFiles]([FileID]) -- Khóa ngoại liên kết với bảng UploadedFiles
);
...
CREATE TABLE [FilePages] (
  [PageID] INT PRIMARY KEY IDENTITY(1,1), -- ID duy nhất cho từng trang trong file
  [FileID] INT NOT NULL, -- ID của file chứa trang này
  [PageNumber] INT NOT NULL, -- Số thứ tự của trang trong file
  [PageContent] VARBINARY(MAX) NOT NULL, -- Nội dung trang sau khi xử lý (dạng nhị phân)
  [IsProcessed] BIT DEFAULT 0, -- Trạng thái trang đã được xử lý hay chưa (1: Đã xử lý, 0: Chưa xử lý)
  [ProcessedDate] DATETIME NULL, -- Ngày trang được xử lý
  FOREIGN KEY ([FileID]) REFERENCES [UploadedFiles]([FileID]) -- Khóa ngoại liên kết với bảng UploadedFiles
);

CREATE TABLE [ProcessedImages] (
  [ProcessedImageID] INT PRIMARY KEY IDENTITY(1,1), -- ID duy nhất cho ảnh đã xử lý
  [FileID] INT NOT NULL, -- ID của file đã được xử lý thành ảnh
  [PageID] INT NOT NULL, -- ID của trang trong file đã xử lý thành ảnh
  [ProcessedImageContent] VARBINARY(MAX) NOT NULL, -- Nội dung ảnh sau khi xử lý (dạng nhị phân)
  [ProcessedDate] DATETIME DEFAULT GETDATE(), -- Ngày xử lý ảnh
  FOREIGN KEY ([FileID]) REFERENCES [UploadedFiles]([FileID]), -- Khóa ngoại liên kết với bảng UploadedFiles
  FOREIGN KEY ([PageID]) REFERENCES [FilePages]([PageID]) -- Khóa ngoại liên kết với bảng FilePages
);
...
CREATE TABLE [FolderFiles] (
  ...
  FOREIGN KEY ([FolderID]) REFERENCES [FolderUploads]([FolderID]), -- Khóa ngoại liên kết với bảng FolderUploads
  FOREIGN KEY ([FileID]) REFERENCES [UploadedFiles]([FileID]) -- Khóa ngoại liên kết với bảng UploadedFiles
);

CREATE TABLE [PDFPages] (
  [PageID] INT PRIMARY KEY IDENTITY(1,1), -- ID duy nhất cho từng trang PDF
  [FileID] INT NOT NULL, -- ID của file PDF chứa trang này
  [PageNumber] INT NOT NULL, -- Số thứ tự của trang trong file PDF
  [OriginalPageContent] VARBINARY(MAX) NOT NULL, -- Nội dung gốc của trang PDF (dạng nhị phân)
  [ProcessedPageContent] VARBINARY(MAX) NULL, -- Nội dung trang PDF sau khi xử lý (nếu có)
  [RotationAngle] INT NULL CHECK (RotationAngle IN (0, 90, 180, 270)), -- Góc xoay của trang PDF (0, 90, 180, 270 độ)
  [IsDeleted] BIT DEFAULT 0, -- Trạng thái trang đã bị xóa hay chưa (1: Đã xóa, 0: Chưa xóa)
  [ActionDate] DATETIME DEFAULT GETDATE(), -- Ngày thực hiện thao tác trên trang PDF
  FOREIGN KEY ([FileID]) REFERENCES [UploadedFiles]([FileID]) -- Khóa ngoại liên kết với bảng UploadedFiles
);
...
CREATE TABLE [FileConversions] (
  ...
  FOREIGN KEY ([DocumentID]) REFERENCES [Document]([DocumentID]), -- Khóa ngoại liên kết với bảng Document
  FOREIGN KEY ([UserID]) REFERENCES [Users]([UserID]), -- Khóa ngoại liên kết với bảng Users
  FOREIGN KEY ([ResultFileID]) REFERENCES [Document]([DocumentID]) -- Khóa ngoại liên kết với bảng Document
);
...
CREATE TABLE [OCRResults] (
  ...
  FOREIGN KEY ([RequestID]) REFERENCES [OCRRequests]([RequestID]) -- Khóa ngoại liên kết với bảng OCRRequests
);
...
CREATE TABLE [FeeHistory] (
  ...
  [FeeDate] DATETIME DEFAULT GETDATE(), -- Ngày tính phí,
  FOREIGN KEY ([FeeID]) REFERENCES RecognitionFees(FeeID) -- Khóa ngoại liên kết với bảng RecognitionFees
);
...
CREATE TABLE [FileDownloads] (
  ...
  FOREIGN KEY ([ResultID]) REFERENCES [OCRResults]([ResultID]) -- Khóa ngoại liên kết với bảng OCRResults
);
